...
class AsyncBaseTest {
	...
	void testCacheNotRegistered(Scheduler scheduler) {
		IllegalStateException exception = assertThrows(IllegalStateException.class, () -> async(scheduler, Mode.ASYNC, Target.BACKGROUND, Cache.NO, 0, ATOMIC_INTEGER_INCREASE, new AtomicInteger(0)));
	...
	void testCacheRegisterUnregister(Scheduler scheduler){
		...
		assertEquals("Cache not registered: "+Cache.NO, exception.getMessage());
		scheduler.register(Cache.NO);
		assertDoesNotThrow(() -> async(scheduler, Mode.ASYNC, Target.BACKGROUND, Cache.NO, 0, ATOMIC_INTEGER_INCREASE, new AtomicInteger(0)));
		scheduler.unregister(Cache.NO);
		exception = assertThrows(IllegalStateException.class, () -> async(scheduler, Mode.ASYNC, Target.BACKGROUND, Cache.NO, 0, ATOMIC_INTEGER_INCREASE, new AtomicInteger(0)));
		assertEquals("Cache not registered: "+Cache.NO, exception.getMessage());
	}
	...
	void testCacheDoubleRegister(Scheduler scheduler){
		scheduler.register(Cache.NO);
		IllegalStateException illegalStateException = assertThrows(IllegalStateException.class, () -> scheduler.register(Cache.NO));
	...
	void testCacheDoubleUnRegister(Scheduler scheduler){
		...
		IllegalStateException illegalStateException = assertThrows(IllegalStateException.class, () -> scheduler.unregister(Cache.NO));
	...
	void testRegisterUnregisterTarget(Scheduler scheduler){
		...
		assertDoesNotThrow(() -> scheduler.register(target,drain::add));
		IllegalStateException illegalStateException = assertThrows(IllegalStateException.class, () -> scheduler.register(target, drain::add));
		assertEquals("Target already registered: "+target, illegalStateException.getMessage());
		assertDoesNotThrow(() -> scheduler.unregister(target));
		IllegalStateException illegalStateException1 = assertThrows(IllegalStateException.class, () -> scheduler.unregister(target));
	...
	void testExecuteAsyncWithoutTarget(Scheduler scheduler){
		...
		Target target = new Target("TestTarget");
		IllegalStateException exception = assertThrows(IllegalStateException.class, () -> async(scheduler, Mode.ASYNC, target, Cache.NO, 0, ATOMIC_INTEGER_INCREASE, new AtomicInteger(0)));
	...
	void testExecutionModeDirect(Scheduler scheduler){
		...
		results.add(async(scheduler, Mode.DIRECT, Target.BACKGROUND, Cache.NO, 0, AsyncTestSetup.WRITE_THREAD_NAME, threadNames));
		results.add(async(scheduler, Mode.DIRECT, Target.BACKGROUND, Cache.NO, 0, AsyncTestSetup.WRITE_THREAD_NAME, threadNames));
		await(results);
	...
	void testExecuteException(Scheduler scheduler){
		setupScheduler(scheduler);
		assertThrows(RuntimeException.class,() ->await(async(scheduler,Mode.DIRECT,Target.BACKGROUND,Cache.NO,0,THROW_EXCEPTION)));
		assertThrows(RuntimeException.class,() ->await(async(scheduler,Mode.ASYNC,Target.BACKGROUND,Cache.NO,0,THROW_EXCEPTION)));
	}
}
